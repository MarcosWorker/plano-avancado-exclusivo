...
async function kjshdbcy(email, senha) {
    const url = 'http://srv443545.hstgr.cloud:3000/api/wiuhw/kljklj';
    const options = {
        method: 'POST',
        headers: {
    ...
    try {
        ...
        console.info('Erro ao fazer login:', error);
        throw new Error('Erro ao fazer login');
    }
}

async function oetwudgs(email, senha) {
    const url = 'http://srv443545.hstgr.cloud:3000/api/wiuhw/reger';
    const options = {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    try {
        ...
        console.info('Erro ao fazer check:', error);
...
async function pzvstdue(email, senha, id, bot) {
    ...
    const url = `http://srv443545.hstgr.cloud:3000/api/wiuhw/${id}/${bot}?${params}`;
    const options = {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
    ...
    try {
        ...
        console.info('Erro ao buscar configurações:', error);
        throw new Error('Erro ao buscar configurações');
    }
}

async function njdiuet(email, senha, usuario_id, bot_tipo, configuracoes) {
    const url = `http://srv443545.hstgr.cloud:3000/api/wiuhw/${usuario_id}/${bot_tipo}`;
    const options = {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    try {
        ...
        console.info('Erro ao criar configuração:', error);
        throw new Error('Erro ao criar configuração');
    }
}

async function iuhytcs(email, senha, usuario_id, bot_tipo, configuracoes) {
    const url = `http://srv443545.hstgr.cloud:3000/api/wiuhw/${usuario_id}/${bot_tipo}`;
    const options = {
        method: 'PUT',
        headers: {
    ...
    try {
        ...
        console.info('Erro ao atualizar configuração:', error);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        case 'oetwudgs':
            actionPromise = oetwudgs(request.email, request.senha);
            break;
        case 'pzvstdue':
            actionPromise = pzvstdue(request.email, request.senha, request.id, request.bot);
            break;
        case 'njdiuet':
            actionPromise = njdiuet(request.email, request.senha, request.id, request.bot, request.config);
            break;
        case 'iuhytcs':
            actionPromise = iuhytcs(request.email, request.senha, request.id, request.bot, request.config);
            break;
        case 'kjshdbcy':
            actionPromise = kjshdbcy(request.email, request.senha);
            break;
        case 'trustedClick':
            actionPromise = clickTrusted(request);
            break;
        default:
            console.info('Ação desconhecida:', request.action);
    ...
    actionPromise
        ...
        .catch(error => {
            console.info('Erro na operação:', error);

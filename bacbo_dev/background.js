...
async function skdouwted(email, senha, usuario_id, bot_tipo, configuracoes) {
...
async function sjdhwue(email, senha, usuario_id, bot_tipo, configuracoes) {
    const url = `https://autobotcasino.com:5000/api/wiuhw/${usuario_id}/${bot_tipo}`;
    const options = {
        method: 'PUT',
        headers: {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        case 'nkosud':
            actionPromise = nkosud(request.email, request.senha);
            break;
        case 'alksjduew':
            actionPromise = alksjduew(request.email, request.senha, request.id, request.bot);
            break;
        case 'skdouwted':
            actionPromise = skdouwted(request.email, request.senha, request.id, request.bot, request.config);
            break;
        case 'sjdhwue':
            actionPromise = sjdhwue(request.email, request.senha, request.id, request.bot, request.config);
            break;
        case 'cisudbc':
            actionPromise = cisudbc(request.email, request.senha);
            break;
        case 'lgiruets':
            actionPromise = lgiruets(request.id);
            break;
        case 'trustedClick':

...
galeInput.addEventListener("input", () => {
    ...
    atualizarMultiplicadoresGale(galeValor);
});
...
cicloInput.addEventListener("input", () => {
    ...
    atualizarCiclos(cicloValor);
});

function atualizarCiclos(qtdCiclos) {
    ...
    for (let i = 0; i < qtdCiclos; i++) {
        ...
        inputGaleQtd.addEventListener("input", () => {
            atualizarGalesNoCiclo(i, parseInt(inputGaleQtd.value, 10));
        });
...
function atualizarGalesNoCiclo(cicloIndex, qtdGales) {
...
function atualizarMultiplicadoresGale(qtdGales) {
...
function adicionarGatilho() {
    ...
    let novoGatilho = {
        ...
        ciclo: ciclos,
        posLoss: getNumericValue(posLossInput.value),
        posGreen: getNumericValue(posGreenInput.value),
        posGain: getNumericValue(posGainInput.value),
        ficha: getNumericValue(inputFicha.value),
        soros: getNumericValue(inputSoros.value),
        cicloSequencial: getNumericValue(inputCicloSequencial.value)
    };
...
function atualizarListaGatilhos() {
    ...
    for (let i = 0; i < gatilhos.length; i++) {
        ...
        listItem.textContent = "Gatilho: " + gatilho.gatilho.join(', ') +
            ...
            " # " + valorSoros(gatilho.soros) +
            " # " + valorCicloSequencial(gatilho.cicloSequencial);
...
function valorCicloSequencial(index) {
    if (index == 0) {
...
async function ajshdaw(email, senha) {
    const response = await enviarMensagem('ajshdaw', { email, senha });
    return response;
}
...
async function kjnvksjn(email, senha, id, bot) {
    const response = await enviarMensagem('kjnvksjn', { email, senha, id, bot, config });
    return response;
}

async function lfkjsdy(email, senha, id, bot) {
    const response = await enviarMensagem('lfkjsdy', { email, senha, id, bot, config });
    return response;
}
